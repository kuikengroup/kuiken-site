...
export default function FileUpload({ clients, projects }: { clients: { id: string; name: string }[]; projects: { id: string; client_id: string; name: string }[] }) {
  ...
  return <form onSubmit={submit} className="grid gap-4 rounded-[2rem] border border-[#E7DCC1]/10 bg-[#211C16] p-7">
    ...
    <select required value={clientId} onChange={(event) => { setClientId(event.target.value); setProjectId(""); }} className={field}><option value="">Choose client</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select>
    ...
    <input value={name} onChange={(event) => setName(event.target.value)} required placeholder="Display name" className={field} />
    ...
    <input ref={fileInput} type="file" required accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.png,.jpg,.jpeg,.zip,.txt" onChange={(event) => { const selected = event.target.files?.[0] ?? null; setFile(selected); if (selected && !name) setName(selected.name); }} />
    ...
    <button disabled={busy || !clients.length} className="rounded-full bg-[#C6A972] px-5 py-3 text-xs font-semibold text-[#17130E] disabled:opacity-50">{busy ? "Uploading…" : "Upload file"}</button>
  </form>;
}
