...
export default async function PortalPage() {
  ...
  return <div className="mx-auto max-w-[90rem] px-6 py-16 lg:px-12">
    ...
    <div className="mt-14 grid gap-4 md:grid-cols-4">{profile.role==="admin"&&<Metric label="Current clients" value={(clientsResult.data??[]).filter(c=>!["completed","archived"].includes(String(c.status))).length}/>}<Metric label="Active projects" value={activeProjects.length}/><Metric label="Open requests" value={openRequests.length}/><Metric label={profile.role==="admin"?"Urgent requests":"Needs your action"} value={requests.filter((row)=>profile.role==="admin"?row.priority==="urgent":row.status==="awaiting_client").length}/></div>
