  ...
  return (
    <section>
      <div className="flex flex-wrap items-end justify-between gap-5">
        ...
        <label className="cursor-pointer rounded-full bg-[#C6A972] px-5 py-3 text-xs font-semibold uppercase tracking-[.16em] text-[#17130E]">
      ...
      {progress !== null && <div aria-label={`Upload ${progress}% complete`} className="mt-4 h-1 overflow-hidden rounded bg-[#382F24]"><div className="h-full bg-[#C6A972] transition-[width]" style={{ width: `${progress}%` }} /></div>}
      {error && <p role="alert" className="mt-4 text-sm text-red-200">{error}</p>}

      <div className="mt-6 flex flex-wrap gap-3">
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search files" aria-label="Search files" className="rounded-xl border border-[#E7DCC1]/10 bg-[#2B241B] px-4 py-3 text-sm" />
        <select value={sort} onChange={(event) => setSort(event.target.value as "new" | "name")} aria-label="Sort files" className="rounded-xl border border-[#E7DCC1]/10 bg-[#2B241B] px-4"><option value="new">Newest</option><option value="name">Name</option></select>
        ...
        <form action={createFolder} className="flex gap-2">
          ...
          <input name="name" required maxLength={80} placeholder="Folder name" aria-label="New folder name" className="w-32 rounded-xl bg-[#2B241B] px-3 text-sm" />
      ...
      <div className={`mt-6 grid gap-3 ${view === "grid" ? "sm:grid-cols-2" : ""}`}>
        {visibleFiles.map((file) => (
          <article key={file.id} className="rounded-2xl border border-[#E7DCC1]/10 bg-[#292219] p-5">
            ...
            <div className="mt-5 flex flex-wrap gap-4 text-xs">
              ...
              <form action={renameFile} className="flex gap-2">
                ...
                <input name="name" aria-label={`Rename ${file.display_name}`} required placeholder="New name" className="w-24 bg-[#382F24] px-2" /><button>Rename</button>
              </form>
