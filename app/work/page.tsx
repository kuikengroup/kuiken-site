      ...
      <Reveal>
        ...
        >
          <div className={`grid gap-12 lg:grid-cols-[1.24fr_0.76fr] lg:items-end ${reverse ? "lg:grid-cols-[0.76fr_1.24fr]" : ""}`}>
            <div className={`image-reveal relative min-h-[30rem] overflow-hidden rounded-[2rem] border border-[#E7DCC1]/10 sm:min-h-[42rem] lg:min-h-[48rem] ${reverse ? "lg:order-2" : ""}`}>
              <Image
                src={project.featuredImage}
                alt={project.imageAlt}
                fill
                sizes="(max-width: 1024px) 100vw, 64vw"
                className="motion-image object-cover group-hover:scale-[1.025]"
              />
  ...
  return (
    <div className="min-h-screen overflow-clip bg-[#0D0D0D] text-[#E7DCC1]">
        ...
        <Container>
          ...
          <div className="mt-20 border-t border-[#E7DCC1]/10 lg:mt-28">
            {[lawnFadez, kuikenGroup, wirelessWorld].map((project, index) => {
              ...
              return (
                <Reveal key={project.slug} stagger={index}>
      ...
      <ConversionCTA
        ...
        title="Your business may be the next story worth telling."
        copy="Start with what feels unclear, outdated, or harder to communicate than it should be."
      />
    </div>
  );
}
