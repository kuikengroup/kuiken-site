...
export default function AuthForm({ mode }: { mode: "login" | "reset" | "update" }) {
  ...
  return <form action={formAction} className="mt-9 grid gap-5">
    {mode !== "update" && <label className="grid gap-2 text-[10px] uppercase tracking-[.2em] text-[#C6A972]">Email<input name="email" type="email" autoComplete="email" required className="rounded-xl border border-[#E7DCC1]/15 bg-[#171817] px-4 py-3 text-base normal-case tracking-normal text-[#E7DCC1]" /></label>}
    {mode !== "reset" && <label className="grid gap-2 text-[10px] uppercase tracking-[.2em] text-[#C6A972]">{mode === "update" ? "New password" : "Password"}<input name="password" type="password" autoComplete={mode === "update" ? "new-password" : "current-password"} minLength={mode === "update" ? 12 : 8} required className="rounded-xl border border-[#E7DCC1]/15 bg-[#171817] px-4 py-3 text-base normal-case tracking-normal text-[#E7DCC1]" /></label>}
    ...
    <button disabled={pending} className="rounded-full bg-[#C6A972] px-6 py-3 text-xs font-semibold uppercase tracking-[.18em] text-[#0D0D0D] disabled:opacity-60">{pending ? "Working…" : mode === "login" ? "Enter Portal" : mode === "reset" ? "Send Reset Link" : "Update Password"}</button>
